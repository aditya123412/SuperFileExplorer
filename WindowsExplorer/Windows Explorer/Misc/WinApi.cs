...
using static Windows_Explorer.Forms.WinApi.Dwm.DWM_BLURBEHIND;
...
public class WinApi
{
    public static void MakeTransparent(IntPtr handle)
    {
        ...
        WinApi.Dwm.WindowSetAttribute(handle, WinApi.Dwm.DWMWINDOWATTRIBUTE.NCRenderingPolicy, (int)policy);
        Version osVersion = Environment.OSVersion.Version;
        if (IsDWNCompositionEnabled())
        {
            if (osVersion.Major > 6)
            {
                WinApi.Dwm.Windows10EnableBlurBehind(handle);
            }
            else if (osVersion.Major == 6 && osVersion.Minor == 1)
            {
                WinApi.Dwm.WindowEnableBlurBehind(handle);
            }
        }
    }

    public static bool IsDWNCompositionEnabled() => (Environment.OSVersion.Version.Major >= 6)
