...
using FileExplorerDesktopClient.ActiveControls;
using FileExplorerDesktopClient.Misc;
using Microsoft.WindowsAPICodePack.Shell;
using Type = FileExplorerDesktopClient.ActiveControls.Type;
...
namespace FileExplorerDesktopClient.Forms
{
    public partial class ExplorerWindow : Form
    {
        ...
        private object RenderFolderNew(string path)
        {
            ...
            List<FileExplorerDesktopClient.C> treeNodes = GetPreRenderMetadata(path, nodesInfo.Select(x => x.Name));
            ArrangeIcons();
            FetchIcons();
            return true;
        }
        ...
        {
            List<FileMeta> treeNodes = new List<FileMeta>();
            var dbTable = TreeDb.GetView(path);
